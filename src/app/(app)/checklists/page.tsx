...
// Per-department checklist templates. Visible to department members; editable
// by department managers. New projects in a department inherit its template.
export default async function ChecklistTemplatesPage({
  ...
  const t = await getTranslations("checklistTemplates");
  ...
  const { data: itemRows } = await supabase
    ...
    .order("sort_order")
    .order("created_at");
  const items = (itemRows ?? []) as ChecklistTemplateRow[];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {canManage ? (
        <p className="text-muted-foreground text-sm">{t("managerNote")}</p>
      ) : (
